...
class Player
{
    ...
    public function setTeam($team, $teamDefault = null)
    {
        if ($this->team == null) {
            ...
            if ($this->team != null) {
                mysqli_query(
                    "UPDATE `players` SET team = '{$this->team}', updated_at = NOW() WHERE id='{$this->mysql_id}'"
                );
            } else {
    ...
    {
        mysqli_query(
            "UPDATE `players` SET pseudo='".mysqli_real_escape_string(
                $this->name
            )."', current_side='".$this->currentSide."' WHERE id='{$this->mysql_id}'"
        ) or Logger::error(mysqli_error());
    }
