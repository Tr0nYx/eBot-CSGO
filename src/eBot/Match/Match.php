...
use eBot\Message\CSGO\RoundScored;
use eBot\Message\Type\Attacked;
use eBot\Message\Type\BombPlanting;
use eBot\Message\Type\ChangeMap;
use eBot\Message\Type\ChangeName;
use eBot\Message\Type\Kill;
use eBot\Message\Type\KillAssist;
use eBot\Message\Type\RoundEnd;
use eBot\Message\Type\TeamScored;
...
class Match implements Taskable
{
    ...
    public function __construct($match_id, $server_ip, $rcon)
    {
        ...
        $this->messageManager = MessageManager::getInstance("CSGO");
        ...
        $query = mysqli_query("SELECT * FROM `matchs` WHERE id = '".$match_id."'");
        if (!$query) {
        ...
        // Get Websocket
        $this->websocket['match'] = Application::getInstance()->getWebSocket('match');
        $this->websocket['livemap'] = Application::getInstance()->getWebSocket('livemap');
        $this->websocket['logger'] = Application::getInstance()->getWebSocket('logger');
        ...
        try {
            ...
            if (!is_numeric(MatchManager::getInstance()->getRetry($this->match_id))) {
                ...
                if (MatchManager::getInstance()->getRetry($this->match_id) > 3) {
                    ...
                    $this->addLog(
                        "Next retry (".MatchManager::getInstance()->getRetry($this->match_id)."/3)"
                    );
                    MatchManager::getInstance()->setRetry(
                        $this->match_id,
                        MatchManager::getInstance()->getRetry($this->match_id) + 1
                    );
                    MatchManager::getInstance()->delayServer($this->server_ip, 5);
                }
            }
            $this->websocket['match']->sendData(
                json_encode(array('message' => 'button', 'content' => 'stop', 'id' => $this->match_id))
            );
        ...
        $query = mysqli_query("SELECT * FROM `maps` WHERE match_id = '".$match_id."'");
        if (!$query) {
            throw new MatchException();
        }
        ...
        if ($this->matchData["current_map"] == null) {
            ...
            foreach ($this->maps as &$map) {
                if (($map->getStatus() == Map::STATUS_NOT_STARTED) && ($map->getMapsFor() == "default")) {
                    ...
                    mysqli_query(
                        "UPDATE `matchs` SET current_map='".$map->getMapId()."' WHERE id='".$this->match_id."'"
                    );
                    break;
                }
            }
        ...
        if ($this->currentMap == null) {
            ...
            mysqli_query(
                "UPDATE `matchs` SET enable='0', status='".self::STATUS_END_MATCH."' WHERE id='".$this->match_id."'"
            );
            throw new MatchException();
        }
        ...
        $this->websocket['match']->sendData(
            json_encode(
                array(
                    'message' => 'teams',
                    'teamA' => $this->side['team_a'],
                    'teamB' => $this->side['team_b'],
                    'id' => $this->match_id,
                )
            )
        );
    ...
    private function recupStatus($eraseAll = false)
    {
        if ($eraseAll) {
            ...
            mysqli_query("DELETE FROM players WHERE map_id='".$this->currentMap->getMapId()."'");
        }
    ...
    public function setStatus($newStatus, $save = false)
    {
        ...
        $this->websocket['match']->sendData(
            json_encode(array('message' => 'status', 'content' => $this->getStatusText(false), 'id' => $this->match_id))
        );
        $this->websocket['match']->sendData(
            json_encode(array('message' => 'button', 'content' => $this->getStatus(), 'id' => $this->match_id))
        );
        if ($save) {
            $this->message = 0;
        ...
        if ($name == self::SET_LIVE) {
            ...
            if (preg_match('/(?<time>\d+\.\d+) seconds/', $tvTimeRemaining, $preg)) {
                TaskManager::getInstance()->addTask(
                    new Task($this, self::TASK_ENGAGE_MAP, microtime(true) + floatval($preg['time']) + 1)
                );
                $this->addLog(
                    "Waiting till GOTV broadcast is finished... Mapchange in ".(intval($preg['time']) + 1)." seconds."
                );
            } else {
                $this->engageMap();
                $this->executeWarmupConfig();
            ...
            if ($this->rcon->getState()) {
                if (!$this->rcon->send('echo eBot')) {
                    ...
                    try {
                        ...
                        $this->addMatchLog(
                            "RCON Connection failed, trying to engage the match in 10 seconds - ".$ex->getMessage(),
                            true
                        );
                        ...
                        MatchManager::getInstance()->delayServer($this->server_ip, 10);
                        $this->needDel = true;
                    }
                }
            }
        ...
        if ((($this->currentMap->getStatus() == Map::STATUS_STARTING) || ($this->currentMap->getStatus() == Map::STATUS_NOT_STARTED)) OR !$this->mapIsEngaged
            ...
            $this->websocket['match']->sendData(
                json_encode(
                    array(
                        'message' => 'currentMap',
                        'mapname' => $this->currentMap->getMapName(),
                        'id' => $this->match_id,
                    )
                )
            );
    ...
    public function sendRotateMessage()
    {
        ...
        if ($this->isMatchRound() && !$this->enable) {
            if (time() - $this->lastMessage >= 8) {
                ...
                $this->say(
                    "Waiting to continue the match - ".$this->formatText(
                        $this->teamAName,
                        "ltGreen"
            ...
            if (time() - $this->lastMessage >= 8) {
                if ($this->getStatus() == self::STATUS_STARTING && $this->timeEngageMap > 0) {
                    ...
                    if ($this->mapIsEngaged && ($this->streamerReady || !$this->config_streamer)) {
                        ...
                        $maps = Config::getInstance()->getMaps();
                        foreach ($maps as $map) {
                            $mapmessage .= "$map, ";
                        }
    ...
    public function say($message, $color)
    {
        ...
        try {
            if (!$this->pluginCsay) {
                $message = str_replace(
                    array("\001", "\002", "\003", "\004", "\005", "\006", "\007"),
                    array("", "", "", "", "", "", ""),
                    $message
                );
    ...
    private function processTeamScored(TeamScored $message)
    {
    ...
    private function processRoundEnd(RoundEnd $message)
    {
        if (!$this->roundEndEvent) {
    ...
    private function processChangeMap(ChangeMap $message)
    {
    ...
    private function processBombPlanting(BombPlanting $message)
    {
        ...
        $this->addMatchLog(
            $this->getColoredUserNameHTML($message->getUserName(), $message->getUserTeam())." planted the bomb."
        );
    ...
    private function processBombDefusing(\eBot\Message\Type\BombDefusing $message)
    {
        ...
        $this->addMatchLog(
            $this->getColoredUserNameHTML($message->getUserName(), $message->getUserTeam())." is defusing bomb."
        );
    ...
    private function processPurchased(\eBot\Message\Type\Purchased $message)
    {
        ...
        if (!$this->waitForRestart && $this->enable && in_array(
            ...
            $text = \addslashes(
                \serialize(
                    array("item" => $message->object, "player" => $user->getId(), "playerName" => $user->get("name"))
                )
            );
...
//          $this->addLog($message->userName . " (" . $message->userTeam . ") purchased " . $message->object);
        ...
        if (preg_match('/\!map (?<mapname>.*)/i', $text, $preg)) {
            ...
            if ($this->pluginCsay) {
                ...
                $this->say_player(
                    ...
                    "Over Time: ".$this->formatText(
                        ...
                        "ltGreen"
                    )." :: MaxRound: ".$this->formatText($this->maxRound, "ltGreen")
                );
            }
            ...
            if ($this->isMatchRound() && !$this->isPaused && $this->enable) {
                ...
                if ($message->getUserTeam() == "CT") {
                    ...
                    if (!$this->pause['ct']) {
                        $this->pause['ct'] = true;
                        if (Config::getInstance()->getPauseMethod() == "instantConfirm") {
                            $this->say($team." (CT) wants to pause, write !pause to confirm.");
                        } elseif (Config::getInstance()->getPauseMethod() == "instantNoConfirm") {
            ...
            if ($this->ready['ct'] && $this->ready['t'] && Config::getInstance()->getDelayReady()) {
                if ($message->getUserTeam() == "CT") {
                    $team = ($this->side['team_a'] == "ct") ? $this->teamAName : $this->teamBName;
            ...
            if ($this->pluginCsay) {
                $this->say_player(
                    ...
                    $this->formatText($this->teamAName, "ltGreen")." ".$this->formatText(
                        ...
                        "green"
                    )." - ".$this->formatText($this->currentMap->getScore2(), "green")." ".$this->formatText(
                        $this->teamBName,
                        "ltGreen"
                    )
                );
        ...
        if (!$this->waitForRestart && $this->enable && in_array(
                            ...
                            if ($this->specialSituation['side'] == "both") {
                                ...
                                if ($this->players[$id]) {
                                    ...
                                    $text = \addslashes(
                                        \serialize(
                                            array(
                                                "situation" => $this->specialSituation['situation'],
                                                "player" => $this->players[$id]->getId(),
                                                "playerName" => $this->players[$id]->get("name"),
                                            )
                                        )
            ...
            if ($bestActionType == "") {
                if ($playerBest != null) {
                    ...
                    $bestActionParam = array(
                        "player" => $playerBest->getId(),
                        "playerName" => $playerBest->get("name"),
                    );
                } else {
                    $bestActionType = null;
            ...
            $this->websocket['match']->sendData(
                json_encode(
                    array('message' => 'status', 'content' => $this->getStatusText(false), 'id' => $this->match_id)
                )
            );
            $this->websocket['match']->sendData(
                json_encode(array('message' => 'button', 'content' => $this->getStatus(), 'id' => $this->match_id))
            ...
            $this->websocket['match']->sendData(
                json_encode(
                    array(
                        'message' => 'score',
                        'scoreA' => $this->score['team_a'],
                        'scoreB' => $this->score['team_b'],
                        'id' => $this->match_id,
                    )
                )
            );
    ...
    private function lookEndingMatch()
    {
         ...
         */
        $delay = 2;
        if (Config::getInstance()->isUseDelayEndRecord()) {
            $delay = 90;
        ...
        if (count($this->maps) == 1 || $allFinish) {
            ...
            if ($this->score["team_a"] > $this->score["team_b"]) {
                $this->say(
                    $this->teamAName." wins! Final score: ".$this->score["team_a"]."/".$this->score["team_b"]."."
                );
                $this->addMatchLog(
                    $this->teamAName." won! Final score: ".$this->score["team_a"]."/".$this->score["team_b"]."."
                );
            } elseif ($this->score["team_a"] < $this->score["team_b"]) {
                $this->say(
                    $this->teamBName." wins! Final score: ".$this->score["team_b"]."/".$this->score["team_a"]."."
                );
                $this->addMatchLog(
                    $this->teamBName." won! Final score: ".$this->score["team_b"]."/".$this->score["team_a"]."."
                );
            } else {
    ...
    private function processChangeName(ChangeName $message)
    {
        $this->processPlayer(
            $message->getUserId(),
            $message->newName,
    ...
    private function generateDamageReports()
    {
        ...
        if (Config::getInstance()->getDamageReportConfig() == false) {
            return;
        }
...
//            $this->say($message->attackerName . " (" . $message->attackerTeam . ") hit " . $message->victimName . " (" . $message->victimTeam . ") for " . $message->attackerDamage . " in round " . $this->getNbRound() . " (hp left: " . $this->roundData[$this->getNbRound()]["HEALTH_LEFT"][$message->victimName] . ") with " . $message->attackerWeapon . " hit in " . $message->attackerHitGroup);
    ...
    private function processKillAssist(KillAssist $message)
    {
        $killer = $this->processPlayer(
            $message->getUserId(),
            $message->getUserName(),
            $message->getUserTeam(),
            $message->getUserSteamid()
...
//        $this->addMatchLog($this->getColoredUserNameHTML($message->userName, $message->userTeam) . " assisted the kill of " . $this->getColoredUserNameHTML($message->killedUserName, $message->killedUserTeam));
    ...
    private function processKill(Kill $message)
    {
        ...
        $this->processPlayer(
            $message->getKilledUserId(),
            $message->getKilledUserName(),
            $message->getKilledUserTeam(),
            $message->getKilledUserSteamid()
        );
    ...
    private function processRoundStart(\eBot\Message\Type\RoundStart $message)
    {
        ...
        if ($this->waitRoundStartRecord) {
            ...
            if (Config::getInstance()->getRememberRecordmsgConfig() != false) {
                for ($i = 0; $i < 3; $i++) {
            ...
            mysqli_query(
                "UPDATE `maps` SET tv_record_file='".$record_name."' WHERE id='".$this->currentMap->getMapId()."'"
            ) or $this->addLog("Error while updating tv record name - ".mysql_error(), Logger::ERROR);
        }
            ...
            if (($this->nbLast['nb_ct'] == 1) && ($this->nbLast['nb_t'] == 1)) {
                ...
                if ($this->nbLast['nb_ct'] == 1) {
                    ...
                    if (($nbAlive > 0) && $id) {
                        ...
                        $this->addMatchLog(
                            "<b>Special situation ! 1v".$nbAlive." (".htmlentities(
                                $this->players[$id]->get("name")
                            ).").</b>"
                        );
                        ...
                        mysqli_query(
                            ...
                            ('".$this->match_id."', '".$this->currentMap->getMapId()."', '1vx', '".addslashes(
                                serialize(
                                    array(
                                        "situation" => $nbAlive,
                                        "player_id" => $this->players[$id]->get("name"),
                                        "player_id" => $this->players[$id]->getId(),
                                    )
                                )
                            )."', '".$this->getRoundTime()."', '".$this->getNbRound()."', NOW(), NOW())
                                "
                        );
                    }
            ...
            if (($this->specialSituation['side2'] != "both") && ($this->specialSituation['side'] != "both")) {
                if (($this->nbLast['nb_ct'] == 1) && ($this->nbLast['nb_t'] == 1)) {
                    if ($this->players[$this->specialSituation['id']]) {
                        $this->addLog(
                            "Special situation 1v1 ! - Player: '".$this->players[$this->specialSituation['id']]->get(
                                "name"
                            )."' is in 1v".$this->specialSituation['situation']."."
                        );
                        $this->addMatchLog(
                            "<b>Special situation 1v1 ! - Player: '".htmlentities(
                                $this->players[$this->specialSituation['id']]->get("name")
                            )."' is in 1v".$this->specialSituation['situation'].".</b>",
                            false
                        );
    ...
    {
        $doPause = false;
        $pauseMethod = Config::getInstance()->getPauseMethod();

        $pauseMethods = array(
            "instantConfirm" => array("text" => "The match is paused.", "method" => "pause"),
            "instantNoConfirm" => array("text" => "The match is paused.", "method" => "pause"),
            "nextRound" => array(
                "text" => "Match will be paused at the start of the next round!",
                "method" => "mp_pause_match",
            ),
        );
        ...
        if ($pauseMethods["$pauseMethod"] && $doPause) {
            ...
            $this->websocket['match']->sendData(
                json_encode(array('message' => 'status', 'content' => 'is_paused', 'id' => $this->match_id))
            );
    ...
    private function unpauseMatch()
    {
        if ($this->unpause["ct"] && $this->unpause["t"] && $this->isMatchRound() && $this->isPaused) {
            ...
            if (Config::getInstance()->getPauseMethod() == "nextRound") {
            ...
            $this->websocket['match']->sendData(
                json_encode(array('message' => 'status', 'content' => 'is_unpaused', 'id' => $this->match_id))
            );
    ...
    private function continueMatch()
    {
        if ($this->continue["ct"] && $this->continue["t"]) {
            ...
            mysqli_query("UPDATE `matchs` SET ingame_enable = 1 WHERE id='".$this->match_id."'") or $this->addLog(
                "Can't update ingame_enable",
                Logger::ERROR
            );
    ...
    private function goLive($type)
    {
        ...
        if (Config::getInstance()->getKo3Method() == "csay" && $this->pluginCsay) {
            $this->rcon->send("csay_".$types["$type"]["restartMethod"]);
        } elseif (Config::getInstance()->getKo3Method() == "esl" && $this->pluginESL) {
    ...
    {
        if (Config::getInstance()->getDelayReady() && !$force_ready && $this->ready['ct'] && $this->ready['t']
        ) {
            ...
            if ($this->ready['ct'] && $this->ready['t']) {
                ...
                if ($this->getStatus() == self::STATUS_WU_KNIFE) {
                    ...
                    if (Config::getInstance()->getConfigKnifeMethod() == "knifestart") {
                        $this->waitRoundStartRecord = true;
                    }
