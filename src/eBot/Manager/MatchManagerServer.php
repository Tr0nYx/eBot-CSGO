...
use eBot\Application\ApplicationServer;
...
class MatchManagerServer extends Singleton implements Taskable
{
            ...
            if (!@$this->matchs[$req['server_ip']]) {
                try {
                    ...
                    if ($ex->getMessage() == "SERVER_BUSY") {
                        Logger::error(
                            $req["server_ip"]." is busy for ".(time() - $this->busyServers[$req['server_ip']])
                        );
    ...
    private function newMatch($match_id, $ip, $rcon, $authkey)
    {
        ...
        if (!@$this->busyServers[$ip]) {
            if (!@$this->matchs[$ip]) {
                ...
                $socket = ApplicationServer::getInstance()->getSocket();
                $socket->sendTo(
                    "addMatch $match_id",
                    Config::getInstance()->getBot_ip(),
                    Config::getInstance()->getBot_port() + $this->matchs[$ip]['i']
                );
    ...
    {
        $instance = ApplicationServer::getInstance()->instance;
        $counter = array();
