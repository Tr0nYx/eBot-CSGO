...
use eBot\Application\ApplicationClient;
...
class MatchManagerClient extends Singleton implements Taskable
{
    ...
    private function check()
    {
        ...
        $sql = mysqli_query(
            ...
            AND m.`status` < ".Match::STATUS_END_MATCH."
            AND m.`enable` = 1"
        ) or die(mysqli_error());
        while ($req = mysqli_fetch_assoc($sql)) {
            if (!@$this->matchs[$req['server_ip']]) {
                try {
                    ...
                    mysqli_query("UPDATE `matchs` SET enable=0 WHERE id = '".$req['match_id']."'")
                    or die(mysqli_error());
                } catch (\Exception $ex) {
