...
class Attacked extends Message
{
    ...
    {
        parent::__construct(
            '/^"(?P<attackerName>.*)[<](?P<attackerUserId>\d+)[>][<](?P<attackerSteamId>.*)[>]'.
            '[<](?P<attackerTeam>CT|TERRORIST|Unassigned|Spectator)[>]" \[(?P<attackerPosX>[\-]?[0-9]+) '.
            '(?P<attackerPosY>[\-]?[0-9]+) (?P<attackerPosZ>[\-]?[0-9]+)\] attacked '.
            '"(?P<victimName>.*)[<](?P<victimUserId>\d+)[>][<](?P<victimSteamId>.*)[>][<]'.
            '(?P<victimTeam>CT|TERRORIST|Unassigned|Spectator)[>]" \[(?P<victimPosX>[\-]?[0-9]+) '.
            '(?P<victimPosY>[\-]?[0-9]+) (?P<victimPosZ>[\-]?[0-9]+)\] with "(?P<attackerWeapon>[a-zA-Z0-9_]+)" '.
            '\(damage "(?P<attackerDamage>[0-9]+)"\) \(damage_armor "(?P<attackerDamageArmor>[0-9]+)"\) '.
            '\(health "(?P<victimHealth>[0-9]+)"\) \(armor "(?P<victimArmor>[0-9]+)"\) '.
            '\(hitgroup "(?P<attackerHitGroup>.*)"\)/'
        );
    }
